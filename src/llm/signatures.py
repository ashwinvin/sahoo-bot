...
class ClassifyQuery(dspy.Signature):
    ...
    Categories:
        ...
        CASUAL: The user is talking casually like a human, Examples: "Hey!", "How are you?"

        ASSIGNMENT_GENERATION: The user is asking to generate a document about a topic which may or may not be in present
            in our database. Examples: "Create a document which explains the experiment I did yesterday"
...
class InfoAgent(dspy.Signature):
    ...
    - If the user has sent the transcription of a voice and if the message seems to be a data dump, then proceed to create
        a summary as the response which has all the data and instruction provided in transcription.
    ...
    - Tools which fetch information from wikipedia shall only be used if the information and message store do not yield
        relevant data. Make sure to always cite the resources used.

    - IF THE USER IS ASKING TO GENERATE A DOCUMENT, YOU SHALL GENERATE CONTENT WHICH WILL BE USED AS THE SOURCE FOR THE FINAL
        DOCUMENT THAT'LL BE GENERATED BY ANOTHER AGENT.
    """
...
class DocumentGenerator(dspy.Signature):
    """You are an assistant who is excellent at creating reports and papers about any topic. The user will ask you to
    create a document for which you may use the given context as source information. You shall format the document using
    markdown
    """
    ...
    file_name: str = dspy.OutputField(
        desc="The name that should set for document when saved as a file, it must have the pdf as its extension"
    )
    sections: list[str] = dspy.OutputField(
        desc="Contents of each section (content which should be in a new page) of the document in markdown format. Each" \
        " section shall start with a title"
    )
    custom_css: str = dspy.OutputField(desc="Custom valid css to style the final pdf")


class ResponsePolisher(dspy.Signature):
    ...
    Make sure to cite the resources used if the category is INFORMATION.

    If the category is `CASUAL`, you shall continue the conversation as such, remain friendly and try to keep the
    conversation alive.

    If the category is "ASSIGNEMENT_GENERATION", you shall only notify the user that the report has been generated and
    uploaded if a file name is provided in proposed answer.
    """
    ...
    output_doc: Optional[str] = dspy.OutputField(
        desc="The name of the generated document mentioned in the proposed answer."
    )
